...
"""
All below to be moved to other repo 
...
def energy_supply_investment_analysis(base_value, end_year, scenario_model_list, df_main, meta_data,
    ...
    """
    variables = ['Investment|Energy Supply','GDP|MER', EconFeas.warming_variable, 'Policy Cost|GDP Loss']
    if region is not None:
        df = df_regional.filter(variable=variables, region=region, year=range(2020, end_year+1), 
                    scenario=scenario_model_list['scenario'], model=scenario_model_list['model'])
        df_temp = df_main.filter(variable=EconFeas.warming_variable, 
            region='World', year=range(2020, end_year+1), scenario=scenario_model_list['scenario'], 
            model=scenario_model_list['model'])
    else:
        df = df_main.filter(variable=variables, region='World', year=range(2020, end_year+1), 
                            scenario=scenario_model_list['scenario'], 
                            model=scenario_model_list['model'])
